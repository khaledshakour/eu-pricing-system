    ...
    if (name.endsWith(".txt") || name.endsWith(".csv") || name.endsWith(".md")) {
      ...
      reader.onload = async (e) => {
        try {
          ...
          reject(new Error("فشل في قراءة ملف PDF: " + err.message));
        }
      };
      reader.readAsArrayBuffer(file);
      ...
      reader.onload = async (e) => {
        try {
          ...
          reject(new Error("فشل في قراءة ملف Word: " + err.message));
        }
      };
      reader.readAsArrayBuffer(file);
    } else {
...
أو: توريد وتركيب نظام إنذار حريق لمستودع مساحة 2000 م²`;
  ...
  const handleFile = async (file) => {
    ...
    if (file.size > maxSize) {
      setFileError("حجم الملف يتجاوز 10 MB");
      return;
    }
    const allowedExts = [".pdf", ".xlsx", ".xls", ".csv", ".txt", ".docx", ".doc", ".md"];
    const ext = "." + file.name.split(".").pop().toLowerCase();
    if (!allowedExts.includes(ext)) {
      setFileError("نوع الملف غير مدعوم. الأنواع المدعومة: PDF, Excel, Word, CSV, TXT");
      return;
    }
    ...
    try {
      ...
      if (!extractedText || extractedText.trim().length === 0) {
        setFileError("لم يتم العثور على نص قابل للقراءة في الملف. جرّب نسخ المحتوى يدوياً في تبويب 'كتابة نصية'.");
      ...
      setFileError(err.message || "حدث خطأ في قراءة الملف");
    }
    setFileLoading(false);
  };
  ...
  const canAnalyze = mode === "file" ? (fileText.trim().length > 0 && !fileLoading) : text.trim().length > 0;
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: 24 }}>
        {mode === "file" ? (
          <div>
            ...
            <input
              ref={fileInputRef}
              type="file"
              accept=".pdf,.xlsx,.xls,.csv,.txt,.docx,.doc,.md"
              style={{ display: "none" }}
              onChange={(e) => {
            ...
            {!uploadedFile ? (
              <div
                ...
                style={{
                  border: `2px dashed ${dragOver ? "#c8973e" : "#d0d8e4"}`,
                  borderRadius: 12,
                  padding: "48px 24px",
                  textAlign: "center",
                  background: dragOver ? "#fff8ec" : "#f8fafc",
                  cursor: "pointer",
                  transition: "all 0.3s",
                }}
              >
                ...
                <div style={{ fontSize: 12, color: "#8896a8", marginTop: 6, fontFamily: "'Tajawal', sans-serif" }}>
                  PDF, Excel, Word, CSV — الحد الأقصى 10 MB
                </div>
                <div style={{
                  marginTop: 16,
              ...
              <div>
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <span style={{ fontSize: 28 }}>{getFileIcon(uploadedFile.name)}</span>
                    <div>
                      <div style={{
                      ...
                      <div style={{
                        ...
                        fontFamily: "'Tajawal', sans-serif",
                      }}>{formatFileSize(uploadedFile.size)}</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    {fileLoading && (
                      <div style={{
                        display: "flex", alignItems: "center", gap: 6,
                        color: "#c8973e", fontSize: 13, fontFamily: "'Tajawal', sans-serif",
                      }}>
                        <div style={{
                          width: 16, height: 16,
                ...
                {fileText && (
                  <div style={{ marginTop: 12 }}>
                    <div style={{
                      ...
                      fontFamily: "'Tajawal', sans-serif", marginBottom: 6,
                    }}>المحتوى المستخرج من الملف (يمكنك تعديله قبل التحليل):</div>
                    <textarea
                      value={text || fileText}
        ...
        <button
          onClick={() => {
            const analyzeText = mode === "file" ? (text || fileText) : text;
            if (analyzeText.trim()) onAnalyze(analyzeText);
          }}
          ...
          style={{
            ...
            gap: 10,
            transition: "all 0.3s",
            boxShadow: canAnalyze ? "0 4px 15px rgba(10,22,40,0.2)" : "none",
          }}
        >
          {loading ? (
            <>
