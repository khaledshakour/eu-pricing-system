      ...
      <div style={{ background: S.white, borderRadius: 12, border: `1px solid ${S.gl}`, overflow: "hidden" }}>
        ...
        <div style={{ padding: 18 }}>
          <input ref={fileRef} type="file" accept=".xlsx,.xls,.csv,.txt" style={{ display: "none" }} onChange={e => {
            ...
            else { r.onload = async ev => { try { const X = await import("https://cdn.sheetjs.com/xlsx-0.20.3/package/xlsx.mjs"); const wb = X.read(new Uint8Array(ev.target.result), { type: "array" }); let t = ""; wb.SheetNames.forEach(s => { t += X.utils.sheet_to_csv(wb.Sheets[s]) + "\n"; }); setInputText(t); } catch(err) { alert("خطأ: " + err.message); } }; r.readAsArrayBuffer(f); }
            e.target.value = "";
          }} />
