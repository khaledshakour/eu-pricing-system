  ...
  try{
    const r=await fetch("https://api.anthropic.com/v1/messages",{
      ...
      body:JSON.stringify({
        ...
        max_tokens:4000,
        messages:[{role:"user",content:`You are a pricing expert for contracting and low current systems in Saudi Arabia (2024-2025 prices).

IMPORTANT SCOPE RULES:
- "Supply & Install" = full material + installation labor cost
- "Installation Only" = ONLY labor cost (20-40% of full price), NO material
- "Supply Only" = ONLY material cost, NO labor

Items to price:
${list}

RESPOND ONLY IN VALID JSON array, no markdown, no backticks, no explanation:
[{"description":"item name","unitPrice":number,"confidence":"high/medium/low","notes":"what is included in this price"}]`}]
      })
    });
    if(!r.ok){console.error("AI API status:",r.status);return null}
    const d=await r.json();
    if(d.error){console.error("AI API error:",d.error);return null}
  ...
  try{
    const r=await fetch("https://api.anthropic.com/v1/messages",{
      ...
      body:JSON.stringify({
        ...
        messages:[{role:"user",content:`Search for current market prices in Saudi Arabia (SAR) for these products. Search for each category:

${items.join("\n")}

After searching, respond ONLY in valid JSON array, no markdown, no backticks:
[{"description":"item name","unitPrice":number,"source":"website or supplier name","notes":"price info found"}]

Focus on Saudi distributors and suppliers. Use SAR currency.`}]
      })
    });
    if(!r.ok){console.error("Web API status:",r.status);return null}
    const d=await r.json();
    if(d.error){console.error("Web API error:",d.error);return null}
